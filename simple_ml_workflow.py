...
class SimpleMLWorkflow:
    ...
    def run_workflow(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"✅ Training completed: {training_results.get('models_trained', 0)} models")
    ...
    def _collect_data(self, config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            clean_dataset, quality_report = self.quality_validator.validate_and_clean_data(dataset)
            ...
            return {
                ...
                'categories': clean_dataset['category'].value_counts().to_dict() if 'category' in clean_dataset.columns else {}
            }

        except Exception as e:
            print(f"❌ Data collection failed: {e}")
            self.database.update_experiment_status(experiment_id, "failed", success=False, error_message=str(e))
    ...
    def _plan_ml_approach(self, data_results: Dict) -> Dict[str, Any]:
        ...
        # Evaluation plan
        evaluation_plan = {
            'metrics': ['accuracy', 'precision', 'recall'] if samples > 30 else ['accuracy'],
            'validation_method': 'train_test_split' if samples > 50 else 'simple_holdout',
    ...
    def _train_simple_models(self, data_results: Dict, planning_results: Dict) -> Dict[str, Any]:
    ...
    def _generate_results_report(self, workflow_results: Dict) -> Dict[str, Any]:
        ...
        report = {
            ...
            'ml_insights': {
                ...
                'best_approach': ml_planning.get('strategies', [{}])[0].get('name', 'None'),
                'training_success_rate': model_training.get('models_trained', 0) / max(len(ml_planning.get('strategies', [])), 1)
            },
            'recommendations': [
        ...
        self.database.save_evaluation(
            model_id=list(model_training.get('training_results', [{}]))[0].get('model_id', 'unknown'),
