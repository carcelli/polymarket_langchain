    ...
    try:
        while time.time() - start_time < duration_minutes * 60:
            ...
            if not updated or not updated.get('active'):
                ...
                import json
                outcome_prices = json.loads(updated.get('outcomePrices', '["0.5", "0.5"]'))
                ...
                print(f"[{timestamp}] Up: {up_price:.3f} | Down: {down_price:.3f} | Vol: ${volume:,.0f}")
...
def main():
    ...
    parser.add_argument(
        '--upcoming',
        action='store_true',
        help='Show upcoming market time slots'
    )
    parser.add_argument(
        '--duration',
        type=int,
    ...
    print(f"⏰ Next Slot: {next_slot.strftime('%H:%M UTC')} ({next_slot.strftime('%I:%M %p ET')})")
