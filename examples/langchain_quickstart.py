...
def example_3_structured_output():
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are a probability forecaster. Analyze the market and return a structured forecast.",
            ),
            ("human", "{query}"),
        ]
    )
...
def example_4_domain_agent():
    ...
    if crypto_domain:
        ...
        print(
            "   (This scans Polymarket for BTC/ETH markets, enriches with live prices,"
        )
...
def example_5_tool_calling_trace():
    ...
    agent = create_polymarket_agent(
        model="gpt-4o-mini", verbose=True, max_iterations=5  # ← This shows tool calls!
    )
...
def example_6_custom_tool():
    ...
    agent = create_polymarket_agent(
        model="gpt-4o-mini", tools=[sentiment_tool], max_iterations=3
    )
