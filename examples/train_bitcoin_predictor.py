...
class BitcoinMarketPredictor:
    ...
    def calculate_edge(
        self, market_prob: float, ml_prob: float, threshold: float = 0.05
    ) -> dict:
        """
        ...
        ev_no = ((1 - ml_prob) * (1 - (1 - market_prob))) - (
            ml_prob * (1 - market_prob)
        )
    ...
    def predict_live_markets(self, db_path: Path, min_edge: float = 0.05):
        ...
        if opportunities:
            ...
            for i, opp in enumerate(opportunities[:10], 1):
                ...
                print(
                    f"   Market Prob: {opp['market_probability']:.1%} | ML Prob: {opp['ml_probability']:.1%}"
                )
                print(f"   Edge: {opp['edge_pct']:+.1f}%")
                print(
                    f"   🎯 {opp['recommendation']} (EV: {opp['expected_value_yes'] if opp['recommendation'] == 'BUY YES' else opp['expected_value_no']:.3f})"
                )
                print()
        else:
...
def main():
    ...
    if df.empty or len(df) < 100:
        ...
        print(
            f"   Need at least 100 resolved markets with quality ≥ {args.min_quality}"
        )
