...
def update_outcomes(conn):
    ...
    for market_id, predicted_dir in open_markets:
        ...
        if winner:
            ...
            c.execute("""UPDATE markets 
                         SET actual_outcome = ?, virtual_profit = ?, status = 'CLOSED' 
                         WHERE id = ?""", 
                      (winner, profit, market_id))
            conn.commit()
            
            print(f"✅ Market {market_id} resolved. Winner: {winner}. Prediction: {predicted_dir}. PnL: ${profit}")
                    ...
                    try:
                        ...
                        if 0 < minutes_until_end < 60:
                            updown_markets.append({
                                'id': market.get('condition_id') or market.get('id'), # Use condition_id or id
                                'question': question,
            ...
            if markets:
                ...
                if new_markets:
                    ...
                    for market in new_markets:
                        ...
                        # Determine Asset
                        asset = 'UNKNOWN'
                        if 'BTC' in market['question']: asset = 'BTC'
                        elif 'ETH' in market['question']: asset = 'ETH'
                        elif 'SOL' in market['question']: asset = 'SOL'
                        ...
                        if predictor:
                            ...
                            if pred_dir:
                                direction = pred_dir
                                confidence = conf
                                print(f"   🤖 Model Prediction: {direction} (Conf: {confidence:.1%})")
                            else:
                        ...
                        try:
                            c.execute("""INSERT INTO markets 
                                         ...
                                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                                      (str(market['id']), asset, 'Unknown', 0.0, direction, confidence, 'OPEN', datetime.now(), market['question'], 0.0))
                            conn.commit()
