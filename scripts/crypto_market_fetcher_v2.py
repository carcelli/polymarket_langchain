...
def fetch_crypto_updown_markets(
    ...
    while True:
        ...
        try:
            ...
            for market in batch:
                ...
                if not ("up or down" in question or (
                    "up" in question and "down" in question and 
                    abs(question.index("up") - question.index("down")) < 30
                )):
                    continue
    ...
    print(f"✅ Fetched {total_fetched} total markets, filtered to {len(markets)} crypto Up/Down")
...
def batch_current_prices(assets: List[str], session: Optional[requests.Session] = None) -> Dict[str, float]:
    ...
    try:
        if session:
            resp = session.get(COINGECKO_URL, params={"ids": ids_param, "vs_currencies": "usd"}, timeout=10)
        else:
            resp = requests.get(COINGECKO_URL, params={"ids": ids_param, "vs_currencies": "usd"}, timeout=10)
...
def print_markets(markets: List[Dict], prices: Optional[Dict[str, float]] = None, max_display: int = 20):
    ...
    for i, market in enumerate(markets[:max_display]):
        ...
        if market['up_price'] is not None and market['down_price'] is not None:
            print(f"    Market odds: UP {market['up_price']:.1%} | DOWN {market['down_price']:.1%}")
        ...
        if market['start_price'] and prices and market['asset'] in prices:
            ...
            print(f"    Price: ${start:,.2f} → ${current:,.2f} ({direction} {abs(change_pct):.2f}%)")
...
Examples:
  ...
  python crypto_market_fetcher_v2.py --json --target-duration 15
    ...
    parser.add_argument('--asset', type=str,
                       help='Filter by asset (BTC, ETH, SOL, XRP, DOGE)')
    parser.add_argument('--json', action='store_true',
                       help='Output as JSON')
    parser.add_argument('--include-expired', action='store_true',
                       help='Include expired markets (for backtesting)')
    parser.add_argument('--no-prices', action='store_true',
                       help='Skip CoinGecko price fetch (faster)')
