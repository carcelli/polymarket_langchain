...
def main():
    if len(sys.argv) < 4:
        print("Usage: python scripts/quick_bet_updown.py <market_id> <UP|DOWN> <amount_usd>")
    ...
    # Confirm
    confirm = input(f"\n⚠️  Execute BET ${amount_usd:.2f} {direction}? (yes/no): ").strip().lower()
    ...
    try:
        ...
        print("   python -c 'from polymarket_agents.langchain.clob_tools import clob_get_open_orders; print(clob_get_open_orders())'")
