...
def fetch_active_sports_markets(category_filter: str = None) -> List[Dict]:
    ...
    try:
        ...
        for market in all_markets:
            ...
            if not ("NBA" in category or "NBA" in question or 
                    "basketball" in question.lower() or
                    any(team in question for team in [
                        "Lakers", "Celtics", "Warriors", "Heat", "Knicks",
                        "76ers", "Suns", "Mavericks", "Nets", "Pistons",
                        "Pacers", "Bucks", "Nuggets", "Clippers"
                    ])):
                continue
...
def extract_game_info(market: Dict) -> Dict:
    ...
    teams = {
        ...
        "Heat": "MIA", "Knicks": "NYK", "76ers": "PHI", "Sixers": "PHI",
        "Suns": "PHX", "Mavericks": "DAL", "Mavs": "DAL",
        "Nets": "BKN", "Pistons": "DET", "Pacers": "IND",
        "Bucks": "MIL", "Nuggets": "DEN", "Clippers": "LAC",
        "Bulls": "CHI", "Raptors": "TOR", "Hawks": "ATL",
        "Hornets": "CHA", "Jazz": "UTA", "Thunder": "OKC",
        "Trail Blazers": "POR", "Blazers": "POR", "Kings": "SAC",
        "Spurs": "SAS", "Timberwolves": "MIN", "Wolves": "MIN",
        "Pelicans": "NOP", "Magic": "ORL", "Wizards": "WAS",
        "Grizzlies": "MEM", "Rockets": "HOU", "Cavaliers": "CLE", "Cavs": "CLE"
    }
    ...
    # Extract date
    date_match = re.search(r'(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}', question)
    ...
    return {
        ...
        "underdog": underdog,
        "date": game_date,
        "matchup": f"{found_teams[0][0]} vs {found_teams[1][0]}" if len(found_teams) >= 2 else "Unknown"
    }
...
def format_market_display(market: Dict) -> str:
    ...
    volume_str = f"${market['volume']/1000:.1f}k" if market['volume'] > 1000 else f"${market['volume']:.0f}"
    ...
    output.append(f"Implied Prob: {market['yes_price']:.1%}" if market['yes_price'] else "")
...
if __name__ == "__main__":
    ...
    print(f"   Highest volume: ${max(m['volume'] for m in markets)/1000:.1f}k" if markets else "")
