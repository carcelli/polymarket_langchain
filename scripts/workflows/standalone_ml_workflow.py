...
class StandaloneMLWorkflow:
    ...
    def run_workflow(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"✅ Strategies planned: {len(planning_results.get('strategies', []))}"
            )
    ...
    def _collect_data(self, config: Dict) -> Dict[str, Any]:
        ...
        for i in range(200):  # Generate 200 mock markets
            ...
            category = np.random.choice(
                ["politics", "sports", "crypto", "economics"], p=[0.4, 0.3, 0.2, 0.1]
            )
        ...
        experiment_results = {
            ...
            "categories": {
                k: int(v) for k, v in df["category"].value_counts().to_dict().items()
            },
    ...
    def _plan_ml_approach(self, data_results: Dict) -> Dict[str, Any]:
        ...
        evaluation_plan = {
            "validation_method": (
                "train_test_split" if samples > 100 else "simple_holdout"
            ),
            "test_size": 0.2,
    ...
    def _train_models(
        ...
        for strategy in strategies:
            ...
            base_accuracy = 0.55 + complexity_bonus.get(
                strategy.get("complexity", "low"), 0
            )
        ...
        training_summary = {
            ...
            "avg_accuracy": np.mean(
                [r["metrics"]["accuracy"] for r in training_results]
            ),
    ...
    def _generate_report(self, workflow_results: Dict) -> Dict[str, Any]:
        ...
        report = {
            ...
            "data_insights": {
                "samples_collected": workflow_results.get("data_collection", {}).get(
                    "samples", 0
                ),
                "quality_score": workflow_results.get("data_collection", {}).get(
                    "quality_score", 0
                ),
                "categories": workflow_results.get("data_collection", {}).get(
                    "categories", {}
                ),
            },
            "ml_insights": {
                "strategies_planned": len(
                    workflow_results.get("ml_planning", {}).get("strategies", [])
                ),
                "models_trained": workflow_results.get("model_training", {}).get(
                    "models_trained", 0
                ),
                "best_accuracy": workflow_results.get("model_training", {})
                .get("summary", {})
                .get("best_accuracy", 0),
            },
            "performance_summary": {
...
def run_standalone_ml_workflow(
...
def show_workflow_database(db_path: str = "data/standalone_ml.db"):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        workflows = conn.execute(
            "SELECT * FROM workflows ORDER BY start_time DESC"
        ).fetchall()
