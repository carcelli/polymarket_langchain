...
class MarketAnalyzer:
    ...
    def get_high_volume_markets(
        self, category: str = None, limit: int = 10, min_volume: float = 1000000
    ) -> List[Dict]:
    ...
    def find_opportunities_by_category(
        self, category: str, min_volume: float = 5000000
    ) -> List[Dict]:
    ...
    def generate_opportunity_report(self, opportunities: List[Dict]) -> str:
        ...
        actionable = [
            ...
            if opp.get("action") in ["BUY", "SELL"] and not opp.get("error")
        ]
        ...
        if not actionable:
            ...
            for opp in opportunities[:5]:  # Show first 5
                status = (
                    "❌ Error"
                    if opp.get("error")
                    else f"⏭️ {opp.get('action', 'UNKNOWN')}"
                )
        ...
        for i, opp in enumerate(actionable[:10], 1):  # Top 10
            ...
            report.append(
                f"   📊 Volume: ${opp['volume']:,.0f} | Implied Prob: {opp['implied_probability']:.1f}%"
            )
            report.append(f"   🎯 Action: {opp['action']} {opp['recommended_side']}")
            report.append(
                f"   📈 Edge: {opp['edge']:.2f}% | EV: {opp['expected_value']:.3f}"
            )
...
def main():
    ...
    print(
        f"Result: {analysis.get('action', 'UNKNOWN')} - Edge: {analysis.get('edge', 0):.2f}%"
    )
    ...
    geopolitics_opportunities = analyzer.find_opportunities_by_category(
        "geopolitics", min_volume=10000000
    )

    if geopolitics_opportunities:
        actionable = [
            opp
            for opp in geopolitics_opportunities
            if opp.get("action") in ["BUY", "SELL"]
        ]
