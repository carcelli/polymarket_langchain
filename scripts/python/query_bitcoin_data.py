...
def get_stats(db_path: Path):
    ...
    # Total snapshots
    total = pd.read_sql_query("SELECT COUNT(*) as count FROM market_snapshots", conn).iloc[0]['count']
    print(f"\n📸 Total Snapshots: {total:,}")
    
    # Unique markets
    markets = pd.read_sql_query("SELECT COUNT(DISTINCT market_id) as count FROM market_snapshots", conn).iloc[0]['count']
    ...
    """, conn)
    print(f"📅 Date Range: {dates.iloc[0]['first'][:10]} to {dates.iloc[0]['last'][:10]}")
    ...
    print(f"⏳ Unresolved Markets: {resolved_stats[resolved_stats['resolved'] == 0]['count'].values[0] if 0 in resolved_stats['resolved'].values else 0:,}")
    ...
    for i, row in top_markets.iterrows():
        ...
        print(f"     Snapshots: {row['snapshots']}, Price Range: {row['min_price']:.3f}-{row['max_price']:.3f}")
...
def export_data(db_path: Path, format: str = 'csv', output_path: str = None):
    ...
    if output_path is None:
        output_path = f"bitcoin_market_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{format}"
...
def get_market_history(db_path: Path, market_id: str):
    ...
    print(f"📅 Period: {df.iloc[0]['timestamp'][:10]} to {df.iloc[-1]['timestamp'][:10]}")
    ...
    for i in range(min(10, len(df))):
        row = df.iloc[-(i+1)]
        print(f"  {row['timestamp'][:19]} | Price: {row['yes_price']:.3f} | BTC: ${row['btc_spot_price']:,.0f}")
...
def get_ml_ready_dataset(
    ...
    feature_cols = [
        'market_probability', 'volume', 'liquidity',
    ...
    if not include_unresolved:
        print(f"   Labels: {df_clean['label_yes'].sum()} YES, {len(df_clean) - df_clean['label_yes'].sum()} NO")
...
def main():
    ...
    parser.add_argument(
        '--export',
        choices=['csv', 'json'],
        help='Export all data to CSV or JSON'
    )
    
    parser.add_argument(
        '--market',
        type=str,
        help='Show history for specific market ID'
    )
    ...
    if not args.db.exists():
        ...
        print(f"   Run the tracker first: python -m polymarket_agents.services.bitcoin_tracker")
        return
