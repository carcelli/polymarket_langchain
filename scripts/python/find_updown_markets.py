...
def main():
    ...
    if market_574073:
        ...
        volume = float(market_574073.get('volume', 0)) if market_574073.get('volume') else 0
        print(f"   Volume: ${volume:,.2f}")
    ...
    for test_id in test_ids:
        ...
        if market and 'question' in market:
            btc_related = any(kw in market.get('question', '').lower() for kw in ['bitcoin', 'btc'])
            if btc_related:
    ...
    if active:
        ...
        for m in sorted(active, key=lambda x: float(x.get('volume', 0)) if x.get('volume') else 0, reverse=True)[:3]:
            print(f"   '{m['id']}'  # {m['question'][:50]}")
