...
def fetch_teams(
    limit: int = 500, league: Optional[str] = None, offset: int = 0
) -> List[Dict]:
...
def display_market_types():
    ...
    categories = {
        ...
        "Esports (FPS)": [
            ...
            "shooter_first_pistol_round",
            "shooter_second_pistol_round",
            "map_handicap",
...
def display_markets_for_league(league: str, limit: int = 10):
    ...
    sports = fetch_sports()
    sport_data = next(
        (s for s in sports if s.get("sport", "").lower() == league.lower()), None
    )
...
Examples:
  ...
  python sports_explorer.py --markets --league nba     # NBA markets
    ...
    parser.add_argument(
        "--leagues", action="store_true", help="List all sports/leagues"
    )
    ...
    parser.add_argument(
        "--market-types", action="store_true", help="List all sports market types"
    )
    parser.add_argument(
        "--league", type=str, help="Filter by league code (e.g., nfl, nba)"
    )
