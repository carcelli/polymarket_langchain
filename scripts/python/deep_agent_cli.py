...
class TodoManager:
    ...
    def manage_todos(self, action: str, task: str = None, index: int = None) -> str:
        if action == "add":
            if not task:
                return "Error: Task description required for 'add'"
            ...
            return "\n".join(
                [
                    f"{i}. [{'x' if t['status']=='done' else ' '}] {t['task']}"
...
class DeepAgentCLI:
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "index": {
                                "type": "integer",
                                "description": "Task index (for complete)",
                            },
                        },
                        "required": ["action"],
                    },
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "topic": {
                                "type": "string",
                                "description": "Memory topic/filename",
                            },
                            "content": {
                                "type": "string",
    ...
    def _handle_tool_call(self, tool_call):
        ...
        if not self.auto_approve:
            self.console.print(
                Panel(
                    f"[bold yellow]Tool Call:[/bold yellow] {name}\n[dim]Args: {args}[/dim]",
        ...
        if self.sandbox_type and name in ["execute_shell", "write_file", "read_file"]:
            self.console.print(
                f"[dim](Executing in {self.sandbox_type} sandbox...)[/dim]"
            )
        ...
        if name == "read_file":
            ...
            return self.todos.manage_todos(
                args["action"], args.get("task"), args.get("index")
            )
    ...
    def chat_loop(self):
        ...
        self.console.print(
            Panel(
                f"[bold cyan]Deep Agents CLI[/bold cyan]{sandbox_info}\nInteractive terminal for building with persistent agents.",
                subtitle=f"Agent: {self.agent_name}",
            )
        )
    ...
    def _handle_slash_command(self, cmd):
        if cmd == "/exit":
            ...
            self.console.print(
                f"[bold]Memories:[/bold] {', '.join(memories) if memories else 'None'}"
            )
        elif cmd == "/tokens":
            self.console.print(
                f"[yellow]Total Tokens Used (Session): {self.total_tokens}[/yellow]"
            )
        else:
        ...
        with Live(
            ...
            while True:
                ...
                for tool_call in msg.tool_calls:
                    ...
                    self.console.print(
                        Panel(
                            f"[bold blue]Observation ({tool_call.function.name}):[/bold blue]\n{display_result}",
                            title="Tool Output",
                        )
                    )
...
def main():
    ...
    parser.add_argument(
        "--auto-approve", action="store_true", help="Skip tool confirmation"
    )
    parser.add_argument(
        "--sandbox",
        choices=["modal", "daytona", "runloop"],
        help="Execute in remote sandbox",
    )
