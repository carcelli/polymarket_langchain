        ...
        for market in markets:
            ...
            if market_id and market_id not in seen_ids:
                ...
                if any(
                    ...
                    for term in ["15m", "15 min", "15-min", "fifteen min"]
                ):
    ...
    for i, market in enumerate(markets, 1):
        ...
        if "outcomes" in market and market["outcomes"]:
            for outcome in market["outcomes"]:
                ...
                print(
                    f"   {outcome.get('outcome', 'Unknown')}: {price:.3f} ({price*100:.1f}%)"
                )

        print()


def export_market_ids(
    markets: List[Dict], output_file: str = "bitcoin_15min_market_ids.json"
):
