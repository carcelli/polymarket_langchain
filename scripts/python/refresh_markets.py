...
def categorize_event(event: Dict) -> str:
    ...
    event_tags = [
        t.get("label", t.get("name", "")).lower() for t in event.get("tags", [])
    ]
...
def cleanup_expired_markets(
    db_path: str = DB_PATH, grace_hours: int = 24
) -> Dict[str, int]:
    """
...
def refresh_database(
    ...
    if cleanup:
        ...
        if cleanup_stats["deleted_count"] > 0:
            print(
                f"     Deleted: {cleanup_stats['deleted_count']:,} markets (${cleanup_stats['deleted_volume']:,.0f} volume)"
            )
    ...
    while offset < max_events:
        ...
        # Progress indicator
        print(
            f"  ✓ Offset {offset}: {len(events)} events, {stats['markets_updated']} markets",
            end="\r",
        )
...
Examples:
    ...
    parser.add_argument(
        "--continuous", "-c", action="store_true", help="Run continuous refresh loop"
    )
    parser.add_argument(
        "--interval",
        "-i",
    ...
    parser.add_argument(
        "--db-path", "-d", type=str, default=DB_PATH, help="Database path"
    )
    parser.add_argument(
        "--no-cleanup", action="store_true", help="Skip removal of expired markets"
    )
    parser.add_argument(
        "--grace-hours",
        "-g",
