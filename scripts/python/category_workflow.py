    ...
    for tag in cat_info["tags"][:5]:  # Limit to avoid too many API calls
        try:
            ...
            if response.status_code == 200:
                ...
                for event in events:
                    event_tags = [
                        t.get("label", "").lower() for t in event.get("tags", [])
                    ]
                    if tag.lower() in event_tags:
...
def print_category_stats(stats: List[Dict]):
    ...
    print()
    print(
        f"  {'TOTAL':<18} │ {'':30} │ {total_markets:>4} markets │ ${total_volume:>12,.0f}"
    )
    print()
...
def main():
    ...
    parser.add_argument(
        "--limit", "-l", type=int, default=100, help="Max events to fetch"
    )
