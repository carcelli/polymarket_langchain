...
def fetch_markets_by_slug(slug: str) -> Optional[Dict]:
    ...
    try:
        ...
        logger.info("fetched_market_by_slug", slug=slug, markets_count=len(event.get('markets', [])))
...
def fetch_crypto_updown_markets(
    ...
    for market in all_markets:
        ...
        if not use_tag_filter:
            is_crypto = any(
                coin in question 
                for coin in ['bitcoin', 'btc', 'ethereum', 'eth', 'solana', 'sol', 'xrp', 'doge']
            )
...
def print_markets(markets: List[Dict], max_display: int = 20) -> None:
    ...
    for i, market in enumerate(markets[:max_display]):
        ...
        if market.get('up_price') and market.get('down_price'):
            print(f"    Odds: UP {market['up_price']:.0%} | DOWN {market['down_price']:.0%}")
        ...
        if start_price and current_price:
            ...
            print(f"    Price: ${start_price:,.2f} → ${current_price:,.2f} ({direction} {abs(change_pct):.2f}%)")
    ...
    logger.info("displayed_markets", total=len(markets), displayed=min(len(markets), max_display))
