...
class NBASimulator:
    ...
    def process_market(self, market: Dict):
        ...
        home_team = info['teams_found'][0] if len(info['teams_found']) > 0 else info['favorite']
        away_team = info['teams_found'][1] if len(info['teams_found']) > 1 else info['underdog']
        ...
        if abs(edge) >= self.min_edge and recommendation == 'BUY':
            self.place_virtual_bet(market, home_team, away_team, winner, model_prob, edge, market_price)
        else:
    ...
    def place_virtual_bet(self, market, home_team, away_team, predicted_winner, model_prob, edge, market_price):
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                if markets:
                    new_markets = [m for m in markets if str(m['id']) not in self.known_markets]
                    
                    if new_markets:
                        print(f"\n{'='*60}")
                        print(f"[{timestamp}] Found {len(new_markets)} new NBA market(s)")
                        print(f"{'='*60}")
                        ...
                        print(f"[{timestamp}] Monitoring... ({self.total_bets} total bets, {win_rate:.1%} win rate)" if self.total_bets > 0 else f"[{timestamp}] Monitoring...")
...
if __name__ == '__main__':
    ...
    print(f"   Customize: python scripts/nba_simulator.py [min_edge] [min_vol] [bet_size] [interval]")
