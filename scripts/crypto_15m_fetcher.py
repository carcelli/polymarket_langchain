...
class Crypto15MinuteFetcher:
        ...
        while keep_fetching:
            ...
            try:
                ...
                if response.status_code == 429:
                    logger.warning(
                        "rate_limit_hit", cooldown_seconds=RATE_LIMIT_COOLDOWN
                    )
                ...
                logger.error("http_error_during_fetch", error=str(e), offset=offset)
                keep_fetching = False
    ...
    def get_arbitrage_candidates(
        self, df: pd.DataFrame, edge_threshold: float = 0.05
    ) -> pd.DataFrame:
        """
            ...
            while True:
                ...
                if not df.empty:
                    ...
                    if not df.empty:
                        ...
                        print(
                            df[
                                ["expiry_min", "event", "yes_price", "volume"]
                            ].to_string(index=False)
                        )
                        print(f"\nTotal: {len(df)} markets")
                    else:
                        print(
                            f"[{datetime.now().strftime('%H:%M:%S')}] No opportunities found."
                        )
                else:
                    print(
                        f"[{datetime.now().strftime('%H:%M:%S')}] No active 15m markets."
                    )
