...
from paper_trading_system import (
    init_database, log_observed_market, place_paper_bet,
    resolve_market, get_performance_summary
)
...
class AutoPaperTrader:
    ...
    def process_market(self, market: dict):
        ...
        if direction and confidence >= self.min_confidence:
            ...
            print(f"   ⏭️  Skipping (confidence {confidence:.1%} < threshold {self.min_confidence:.1%})")
    ...
    def check_for_resolutions(self):
        ...
        for market_id, bet_info in self.active_bets.items():
            try:
                end_time = datetime.fromisoformat(bet_info['end_time'].replace('Z', '+00:00'))
        ...
        try:
            while True:
                ...
                if markets:
                    ...
                    if new_markets:
                        print(f"\n{'='*60}")
                        print(f"[{current_time}] Found {len(new_markets)} new market(s)!")
                        print(f"{'='*60}")
                        ...
                        print(f"[{current_time}] Monitoring... ({len(self.active_bets)} active bets)")
...
if __name__ == '__main__':
    ...
    print(f"   To customize: python scripts/auto_paper_trader.py [bet_size] [min_confidence] [interval]")
