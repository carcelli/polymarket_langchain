...
def main():
    if len(sys.argv) < 4:
        print(
            "Usage: python scripts/quick_bet.py <market_id> <outcome_index> <amount_usd>"
        )
    ...
    token_id = (
        ...
        else token_ids.split(",")[outcome_index]
    )
    outcome_name = (
    ...
    # Confirm
    confirm = (
        input(f"\n⚠️  Execute BET ${amount_usd:.2f} on '{outcome_name}'? (yes/no): ")
        .strip()
        .lower()
    )
