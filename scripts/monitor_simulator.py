...
class Simulator:
        ...
        try:
            ...
            for token in tokens:
                ...
                if price > 0.95:  # Winner has price ~$1
                    if 'UP' in outcome_name or 'YES' in outcome_name or 'HIGHER' in outcome_name:
                        return {'outcome_dir': 'UP', 'actual_end_price': price}
                    elif 'DOWN' in outcome_name or 'NO' in outcome_name or 'LOWER' in outcome_name:
        ...
        try:
            while True:
                ...
                if check_count % 10 == 0:
                    ...
                    print(f"[{timestamp}] Monitoring... ({unresolved_bets} pending, {self.total_bets} resolved)")
    ...
    def print_summary(self):
        ...
        win_rate = self.winning_bets / self.total_bets if self.total_bets > 0 else 0
        drawdown = (self.max_bankroll - self.bankroll) / self.max_bankroll if self.max_bankroll > 0 else 0
        
        print(f"\n💰 Portfolio:")
        print(f"   Starting bankroll: $1,000.00")
        print(f"   Current bankroll: ${self.bankroll:.2f}")
        print(f"   Total P&L: ${self.total_profit:+.2f} ({self.total_profit/1000*100:+.1f}%)")
...
if __name__ == '__main__':
    ...
    print(f"   Customize: python scripts/monitor_simulator.py [min_conf] [risk] [interval]")
