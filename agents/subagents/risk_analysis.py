...
def calculate_portfolio_risk(positions: List[Dict[str, Any]], total_portfolio: float = 100000) -> Dict[str, Any]:
    """
    ...
    if not positions:
        return {
            "error": "No positions provided for risk analysis"
        }
    ...
    diversification_ratio = len(position_risks) / max(1, total_exposure / total_portfolio)
    concentration_limit = max((pos['market_value'] / total_portfolio) for pos in position_risks)
    ...
    risk_analysis = {
        ...
        "risk_metrics": {
            ...
            "risk_concentration": "High" if concentration_limit > 0.2 else "Medium" if concentration_limit > 0.1 else "Low"
        },
...
def calculate_kelly_position_size(edge: float, odds: float, bankroll: float, kelly_fraction: float = 1.0) -> Dict[str, Any]:
    """
    ...
    # Risk assessment
    risk_level = "Low" if kelly_fraction_used < 0.02 else "Medium" if kelly_fraction_used < 0.05 else "High"
...
def assess_market_risk(market_data: Dict[str, Any], position_size: float) -> Dict[str, Any]:
    """
    ...
    liquidity_risk = "Low" if liquidity > 50000 else "Medium" if liquidity > 10000 else "High"
    volatility_risk = "Low" if volatility < 0.3 else "Medium" if volatility < 0.7 else "High"
    ...
    overall_risk = "High" if risk_score > 2.5 else "Medium" if risk_score > 1.5 else "Low"

    # Position size recommendations
    max_position_pct = 0.05 if overall_risk == "High" else 0.10 if overall_risk == "Medium" else 0.20
