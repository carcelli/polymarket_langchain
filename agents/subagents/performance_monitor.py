...
def generate_performance_report(trades: List[Dict[str, Any]], time_period: str = "30d") -> Dict[str, Any]:
    """
    ...
    avg_win = sum(t.get('pnl', 0) for t in filtered_trades if t.get('pnl', 0) > 0) / max(1, winning_trades)
    avg_loss = sum(t.get('pnl', 0) for t in filtered_trades if t.get('pnl', 0) < 0) / max(1, losing_trades)
    ...
    if daily_returns:
        ...
        std_daily_return = (sum((r - avg_daily_return)**2 for r in daily_returns) / max(1, len(daily_returns)))**0.5
        sharpe_ratio = avg_daily_return / max(0.001, std_daily_return) * (252**0.5)  # Annualized
    else:
        sharpe_ratio = 0
    ...
    performance_report = {
        ...
        "trade_analysis": {
            "avg_win": avg_win,
            "avg_loss": avg_loss,
            "profit_factor": abs(sum(t.get('pnl', 0) for t in filtered_trades if t.get('pnl', 0) > 0) /
                               max(1, sum(t.get('pnl', 0) for t in filtered_trades if t.get('pnl', 0) < 0))),
    ...
    if total_pnl < 0:
        performance_report["alerts"].append("Negative P&L - consider strategy adjustments")
    if sharpe_ratio < 0.5:
        performance_report["alerts"].append("Poor risk-adjusted returns - increase edge or reduce risk")
...
def analyze_market_exposure(positions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if positions:
        ...
        concentration_pct = largest_position.get('market_value', 0) / max(1, total_exposure)
    else:
        concentration_pct = 0
    ...
    exposure_analysis = {
        ...
        "concentration_analysis": {
            ...
            "concentration_level": "High" if concentration_pct > 0.3 else "Medium" if concentration_pct > 0.15 else "Low",
        ...
        "recommendations": [
            ...
            "Consider rebalancing if concentration exceeds 30%" if concentration_pct > 0.3 else "Diversification looks good"
        ]
    }
