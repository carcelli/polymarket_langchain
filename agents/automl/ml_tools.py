...
class DataIngestionInput(BaseModel):
    ...
    days_back: int = Field(description="Number of days of historical data to fetch", default=365)
    min_volume: float = Field(description="Minimum market volume threshold", default=1000)
    include_unresolved: bool = Field(description="Whether to include unresolved markets", default=False)
...
class RunAutoMLPipelineInput(BaseModel):
    ...
    models_to_train: List[str] = Field(description="Models to train", default=["MarketPredictor", "EdgeDetector"])


class TrainMLModelInput(BaseModel):
    """Input schema for model training tool."""
    model_type: str = Field(description="Type of model to train (MarketPredictor, EdgeDetector)")
    experiment_name: str = Field(description="Experiment name for tracking")
    hyperparameters: Optional[Dict[str, Any]] = Field(description="Model hyperparameters", default=None)
...
class EvaluateModelInput(BaseModel):
    ...
    evaluation_type: str = Field(description="Type of evaluation (backtest, cross_validation)", default="backtest")
...
class DataIngestionTool(BaseTool):
    ...
    description: str = "Ingest historical market data from Polymarket and prepare it for ML training"
    args_schema: Type[BaseModel] = DataIngestionInput

    def _run(self, days_back: int = 365, min_volume: float = 1000,
             include_unresolved: bool = False, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            dataset_id = db.save_dataset(
                ...
                target_distribution=dataset['will_resolve_yes'].value_counts().to_dict()
            )
...
class DataQualityTool(BaseTool):
    ...
    def _run(self, dataset_info: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
...
class AutoMLPipelineTool(BaseTool):
    ...
    def _run(self, experiment_name: str, days_back: int = 365, min_volume: float = 1000,
             models_to_train: List[str] = None, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            if results['success']:
                return json.dumps({
                    ...
                    "pipeline_duration": results['pipeline_metadata']['duration_seconds'],
...
class ModelTrainingTool(BaseTool):
    ...
    def _run(self, model_type: str, experiment_name: str, hyperparameters: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            # Train model
            train_data = dataset[['market_id', 'question', 'category', 'yes_price',
                                'no_price', 'volume', 'liquidity', 'actual_outcome']].copy()
            train_data = train_data.rename(columns={'actual_outcome': 'will_resolve_yes'})
            ...
            model_info = {
                ...
                'feature_columns': list(model.feature_columns) if hasattr(model, 'feature_columns') else [],
    ...
    def _calculate_metrics(self, predictions_df: pd.DataFrame) -> Dict[str, float]:
        ...
        # Binary predictions
        valid_predictions['pred_binary'] = (valid_predictions['predicted_probability'] > 0.5).astype(int)

        try:
            metrics = {
                'accuracy': accuracy_score(valid_predictions['actual_outcome'], valid_predictions['pred_binary']),
                'precision': precision_score(valid_predictions['actual_outcome'], valid_predictions['pred_binary'], zero_division=0),
                'recall': recall_score(valid_predictions['actual_outcome'], valid_predictions['pred_binary'], zero_division=0),
                'f1': f1_score(valid_predictions['actual_outcome'], valid_predictions['pred_binary'], zero_division=0),
                'roc_auc': roc_auc_score(valid_predictions['actual_outcome'], valid_predictions['predicted_probability']),
...
class ModelEvaluationTool(BaseTool):
    ...
    def _run(self, model_id: str, evaluation_type: str = "backtest",
             run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            with db._ensure_database():  # Ensure connection
                ...
                model_dict = dict(zip([desc[0] for desc in model_info.description], model_info))
            ...
            # Get test data
            ingestion = PolymarketDataIngestion()
            test_data = ingestion.create_training_dataset(days_back=180, min_volume=2000)
            ...
            db.save_evaluation(model_id, evaluation_type, evaluation_config, results, duration)
    ...
    def _run_backtest_evaluation(self, model, test_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    def _run_cross_validation(self, model, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        feature_cols = [col for col in data.columns if col not in ['will_resolve_yes', 'market_id', 'question']]
...
class PredictionTool(BaseTool):
    ...
    def _run(self, model_id: str, market_data: Dict[str, Any],
             run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
...
class ExperimentResultsTool(BaseTool):
    ...
    def _run(self, experiment_id: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            # Add best model info
            if results['models']:
                best_model = max(results['models'], key=lambda m: m.get('metrics', [{}])[0].get('value', 0) if m.get('metrics') else 0)
