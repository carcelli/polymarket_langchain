...
class PolymarketDataIngestion:
    ...
    def fetch_historical_markets(self, days_back: int = 365, limit: int = 10000) -> List[Dict]:
        """
        ...
        try:
            ...
            response_closed = requests.get(closed_url, headers=self.headers, params=params_closed)
        ...
        for market in raw_markets:
            try:
                ...
                if isinstance(outcome_prices, str):
                    outcome_prices = [float(p.strip()) for p in outcome_prices.strip('[]').split(',') if p.strip()]
                ...
                if resolved and winner is not None:
                    ...
                    if isinstance(winner, str):
                        actual_outcome = 1 if winner.lower() in ['yes', 'true', '1'] else 0
                ...
                logger.warning(f"Failed to clean market {market.get('id', 'unknown')}: {e}")
                continue
    ...
    def engineer_ml_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ml_df['price_volatility'] = ml_df['yes_price'] * (1 - ml_df['yes_price'])  # Information entropy
        ml_df['price_extremity'] = (ml_df['yes_price'] < 0.2) | (ml_df['yes_price'] > 0.8)
        ...
        ml_df['days_until_end'] = ml_df['days_until_end'].clip(lower=0)  # Don't go negative

        # Category features (one-hot would be done in model preprocessing)
        major_categories = ['politics', 'sports', 'crypto', 'geopolitics', 'tech', 'economics']
        for cat in major_categories:
            ml_df[f'is_{cat}'] = ml_df['category'].str.contains(cat, case=False, na=False)
        ...
        ml_df['has_crypto_keywords'] = ml_df['question'].str.contains(
            r'bitcoin|ethereum|crypto|blockchain|nft|defi',
            case=False, regex=True
        )

        # Market microstructure features
        ml_df['spread'] = abs(ml_df['yes_price'] - ml_df['no_price'])
        ml_df['market_efficiency_score'] = 1 / (ml_df['spread'] + 0.01)  # Lower spread = more efficient
    ...
    def create_training_dataset(self, days_back: int = 365, min_volume: float = 1000,
                               include_unresolved: bool = False) -> pd.DataFrame:
        """
        ...
        logger.info(f"Final ML dataset: {len(ml_dataset)} samples, {len(ml_dataset.columns)} features")

        # Cache the dataset
        cache_file = self.cache_dir / f"ml_training_data_{datetime.now().strftime('%Y%m%d')}.parquet"
        ...
        try:
            ...
            for _, row in ml_dataset.iterrows():
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    row['market_id'], row['question'], row['category'],
                    row['yes_price'], row['no_price'], row['volume'], row['liquidity'],
                    row['resolved'], row['actual_outcome'],
                    row.get('created_at'), row.get('end_date'), row.get('resolved_at'),
                    row.get('days_to_resolve'),
                    row.get('price_distance_from_fair'), row.get('price_volatility'),
                    row.get('log_volume'), row.get('market_age_days'),
                    row.get('question_word_count'), row.get('has_political_keywords'),
                    row.get('has_sports_keywords'), row.get('has_crypto_keywords'),
                    row.get('market_efficiency_score'), row.get('will_resolve_yes')
                ))
