...
from ..subagents.github_ml_agent import generate_ml_strategy_test, commit_ml_tests_to_github
...
class AutoMLPipeline:
    ...
    def run_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            reports = self._generate_reports_and_tests(ml_dataset, model_results, best_model_info)
    ...
    def _validate_and_clean_data(self, dataset: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
        """Step 2: Validate and clean data quality."""
        quality_report = self.quality_validator.validate_dataset_structure(dataset)

        if quality_report['issues']:
            logger.warning(f"⚠️ Found {len(quality_report['issues'])} data quality issues")
    ...
    def _add_automl_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'category' in df.columns:
            df['category_volume_avg'] = df.groupby('category')['volume'].transform('mean')
            df['category_price_avg'] = df.groupby('category')['yes_price'].transform('mean')
    ...
    def _calculate_model_metrics(self, predictions_df: pd.DataFrame) -> Dict[str, float]:
        ...
        metrics = {}
        try:
            metrics['accuracy'] = accuracy_score(predictions_df['actual'], predictions_df['pred_binary'])
            metrics['precision'] = precision_score(predictions_df['actual'], predictions_df['pred_binary'], zero_division=0)
            metrics['recall'] = recall_score(predictions_df['actual'], predictions_df['pred_binary'], zero_division=0)
            metrics['f1'] = f1_score(predictions_df['actual'], predictions_df['pred_binary'], zero_division=0)
            metrics['roc_auc'] = roc_auc_score(predictions_df['actual'], predictions_df['prediction'])
        except Exception as e:
    ...
    def _select_best_model(self, model_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"🏆 Selected {best_model_name} as best model (score: {model_scores[best_model_name]:.4f})")

        return best_model_info

    def _generate_reports_and_tests(self, dataset: pd.DataFrame, model_results: Dict,
                                   best_model_info: Dict) -> Dict[str, Any]:
        """Step 6: Generate comprehensive reports and automated tests."""
        reports = {}

        # 1. Performance Report
        performance_report = self._generate_performance_report(model_results, best_model_info)
        ...
        for report_name, report_data in reports.items():
            report_file = self.reports_dir / f"{report_name}_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _generate_performance_report(self, model_results: Dict, best_model_info: Dict) -> Dict:
        ...
        return {
            ...
            'model_comparison': {
                name: result.get('metrics', {}) for name, result in model_results.items()
                if 'error' not in result
            },
            'dataset_info': {
                'training_samples': list(model_results.values())[0].get('training_samples', 0),
                'features_used': len(list(model_results.values())[0].get('feature_importance', {}))
            },
    ...
    def _generate_model_recommendations(self, best_model_info: Dict) -> List[str]:
        ...
        if metrics.get('accuracy', 0) < 0.6:
            recommendations.append("Model accuracy is below 60% - consider more features or different algorithm")
        elif metrics.get('accuracy', 0) > 0.8:
            recommendations.append("Excellent model accuracy - ready for production deployment")

        if metrics.get('f1', 0) < 0.5:
            recommendations.append("Poor F1 score - model may be imbalanced or overfitting")

        if metrics.get('roc_auc', 0) > 0.8:
            recommendations.append("Strong discriminative ability - model effectively separates classes")
    ...
    def _deploy_best_model(self, best_model_info: Dict) -> Dict[str, Any]:
        ...
        # Save model
        model_file = self.models_dir / f"{model_name}_automl_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
        try:
    ...
    def _save_pipeline_results(self, results: Dict[str, Any]) -> None:
        ...
        results_file = self.output_dir / f"automl_pipeline_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def predict_with_deployed_model(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
