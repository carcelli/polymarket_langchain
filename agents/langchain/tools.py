...
class EventQueryInput(BaseModel):
    ...
    tradeable_only: bool = Field(default=True, description="Only return tradeable events")
...
class NewsQueryInput(BaseModel):
    ...
    date_start: Optional[str] = Field(default=None, description="Start date (YYYY-MM-DD)")
...
def _get_superforecast_impl(event_title: str, market_question: str, outcome: str) -> str:
...
def _query_markets_rag_impl(query: str, db_directory: str = "./local_db_markets") -> str:
