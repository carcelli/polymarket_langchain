...
def _get_github_toolkit() -> Optional[GitHubToolkit]:
    ...
    try:
        ...
        if not all(
            os.getenv(var)
            for var in ["GITHUB_APP_ID", "GITHUB_APP_PRIVATE_KEY", "GITHUB_REPOSITORY"]
        ):
...
get_issues = wrap_tool(
    _get_issues_impl, name="get_issues", description="Fetch issues from the repository."
)
get_issue = wrap_tool(
    _get_issue_impl,
    name="get_issue",
...
create_issue_comment = wrap_tool(
