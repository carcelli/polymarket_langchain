...
def _execute_market_order_impl(token_id: str, amount: float, side: str) -> str:
    ...
    try:
        ...
        if not poly or not poly.client:
            return json.dumps(
                {"error": "Polymarket client not initialized (check private key)"}
            )
        ...
        if side.upper() == "BUY":
            ...
            order_args = OrderArgs(
                price=0.01, size=amount, side=SELL, token_id=token_id
            )
...
def _execute_limit_order_impl(
    ...
    try:
        ...
        resp = poly.client.create_and_post_order(
            OrderArgs(price=price, size=size, side=side_const, token_id=token_id)
        )
        return json.dumps(resp, default=str)
