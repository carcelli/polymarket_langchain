...
class KNNEdgeDetector(MLBettingStrategy):
    ...
    def predict(self, market_data: Dict[str, Any]) -> StrategyResult:
        if (
            not hasattr(self, "is_trained")
            or not self.is_trained
            or self.historical_data is None
        ):
        ...
        if total_weight > 0:
            confidence = max(
                weights.most_common(1)[0][1] / total_weight if weights else 0.5, 0.5
            )
        else:
            confidence = 0.5
        ...
        recommendation = (
            "BUY_YES"
            if pred_prob > 0.5 + edge / 2
            else "BUY_NO" if pred_prob < 0.5 - edge / 2 else "HOLD"
        )
