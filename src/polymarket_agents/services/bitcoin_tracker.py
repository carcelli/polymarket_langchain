...
class BitcoinMarketTracker:
    ...
    def _fetch_bitcoin_markets(self) -> List[Dict]:
        ...
        try:
            ...
            for m in markets:
                ...
                if any(keyword in question.lower() for keyword in ['bitcoin', 'btc', '$btc']):
                    # Prioritize price prediction markets
                    if any(word in question.lower() for word in ['reach', 'hit', 'above', 'below', 'price', '$']):
                        btc_markets.append(m)
    ...
    def _save_snapshot(self, market: Dict, btc_data: Optional[Dict]) -> bool:
        ...
        try:
            ...
            yes_price = float(tokens[0]['price']) if len(tokens) > 0 and tokens[0].get('price') else None
            no_price = float(tokens[1]['price']) if len(tokens) > 1 and tokens[1].get('price') else None
            ...
            if end_date:
                try:
                    ...
                    time_to_expiry_hours = (expiry - datetime.now(timezone.utc)).total_seconds() / 3600
                except:
                    pass
            ...
            indicators = self._calculate_technical_indicators(market_id, yes_price, volume)
            ...
            """, (
                timestamp, market_id, question,
                yes_price, no_price, yes_price,  # implied_probability = yes_price
                volume, liquidity, end_date, time_to_expiry_hours,
    ...
    def collect_snapshot(self) -> int:
        ...
        if btc_data:
            logger.info(f"₿  BTC: ${btc_data['price']:,.2f} ({btc_data['change_24h_pct']:+.2f}% 24h)")
        ...
        for market in markets:
            if self._save_snapshot(market, btc_data):
                ...
                if market_id in self.last_prices:
                    current = float(market['tokens'][0]['price']) if market.get('tokens') else 0
                    last = self.last_prices[market_id]
                    change = current - last
                    if abs(change) > 0.01:  # >1% change
                        logger.info(f"   📈 {market['question'][:60]}... ({change:+.3f})")
    ...
    def start(self, run_once: bool = False):
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE id = ?
            """, (datetime.now(timezone.utc).isoformat(), total_snapshots, total_errors, run_id))
            self.conn.commit()
            ...
            logger.info(f"🎉 Tracker stopped. Collected {total_snapshots} snapshots ({total_errors} errors)")
